...
Форк [pi-mono](https://github.com/badlogic/pi-mono), стремящийся достичь уровня [claude-code](https://github.com/anthropics/claude-code) и превзойти его. Оригинальный pi минималистичен и ему не хватает многих вещей для комфортной повседневной работы. Цель pi-x — исправить это, сохранив модульную архитектуру, заложенную автором оригинала.

Особая благодарность [Mario Zechner](https://github.com/badlogic) за модульную архитектуру pi-mono, которая сделала возможным создание pi-x.
...
Режим Vim-подобного редактирования ввода с тремя состояниями:

- **INSERT** — обычный ввод текста (как без Vim)
- **NORMAL** — навигация и операции: `h/j/k/l`, `w/b/e`, `0/$`, `gg/G`, `dd/cc/yy`, `d/c/y` + motion, `u` (undo), `Ctrl+R` (redo)
- **VISUAL** — выделение через `v` (посимвольно) или `V` (построчно), операции `d`/`y`/`c` над выделением

Включается через `/vim` или настройку `vimModeEnabled` в `settings.json`. Состояние сохраняется между сессиями.

В Vim mode `Ctrl+c` и `Ctrl+d` сразу закрывают pi.

### YOLO Mode

Сессионный режим автоматического разрешения всех операций (кроме критических блокировок: `rm -rf /` и т.д.). Включается `/yolo`, отображается в footer как `[YOLO]`.

### Команда /btw

Out-of-band вопросы к агенту без добавления в историю сессии:

```
/btw объясни что делает этот промпт
```

Агент отвечает, используя контекст текущей сессии, но вопрос и ответ не попадают в историю диалога.

### Поиск истории промптов

`/search` или `Ctrl+f` открывает интерактивный селектор с fuzzy-фильтрацией по истории введённых промптов. Выбор подставляет промпт в редактор.
...
| `/yolo` | Включить/выключить YOLO-режим |
| `/vim` | Включить/выключить Vim-режим |
| `/btw <вопрос>` | Out-of-band вопрос к агенту |
| `/search` | Поиск по истории промптов |
...
[Mario Zechner](https://github.com/badlogic) — автор оригинального [pi-mono](https://github.com/badlogic/pi-mono), чья модульная архитектура позволила создать pi-x.
